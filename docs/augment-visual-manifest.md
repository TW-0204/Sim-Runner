...
| G13 | 내일의 나에게 | Gold | either | Store one unused movement result until the next turn; the stored movement distance increases by 1. If the restored result has no legal piece that can use it, it expires under the normal unusable-result rule. | move result placed in storage, then returned stronger |
