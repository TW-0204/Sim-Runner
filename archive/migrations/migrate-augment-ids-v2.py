...
def write_legacy_map():
    lines = ["export const LEGACY_AUGMENT_ID_MAP = {"]
...
def migrate_catalog():
    ...
    text = text.replace('  requires?: string;\n', '  requires?: AugmentId;\n')
    text = text.replace('  conflicts?: string[];\n', '  conflicts?: AugmentId[];\n')
    ...
    old_tail = 'export const AUGMENT_BY_ID = new Map(AUGMENTS.map((augment) => [augment.id, augment]));\n\nexport function getAugment(id: string) {\n  return AUGMENT_BY_ID.get(id) ?? null;\n}\n'
    new_tail = '''const LEGACY_ALIASES_BY_CANONICAL = new Map<AugmentId, string[]>();
...
/** Current natural/offer pool. Retired augments remain in AUGMENT_CATALOG for history only. */
export const AUGMENTS = AUGMENT_CATALOG.filter((augment) => augment.status === "active");
...
'''
    ...
    CATALOG_PATH.write_text(text.replace(old_tail, new_tail), encoding="utf-8")

def remove_dead_unlisted_ids():
    path = Path("src/lib/augments/effects.ts")
    text = path.read_text(encoding="utf-8")
    text = text.replace('  if (source === "CAPTURE" && has(ownedIds, "P01")) {\n    transformed = "YUT";\n  } else if (source === "BASIC") {\n', '  if (source === "BASIC") {\n')
    text = text.replace('  if (source === "CAPTURE" && face !== "BACKDO") {\n    if (has(ownedIds, "G02")) steps += 2;\n    else if (has(ownedIds, "AUG-001")) steps += 1;\n  }\n', '  if (source === "CAPTURE" && face !== "BACKDO" && has(ownedIds, "AUG-001")) {\n    steps += 1;\n  }\n')
...
def update_agents():
    ...
    text = text.replace('2. Create `src/lib/augments/runtime/<ID>.ts` exporting an `implementation: "hooked"` runtime registration.\n', '2. Create `src/lib/augments/runtime/<AUG-###>.ts` exporting an `implementation: "hooked"` runtime registration.\n')
...
def main():
    ...
    print(f"migrated {len(MAPPING)} augment identities to canonical AUG ids")
