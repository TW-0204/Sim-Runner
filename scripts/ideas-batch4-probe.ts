...
assert.equal(AUGMENT_BY_ID.get("AUG-056")?.tier, "prism");

// Acquisition opens AUG-056 immediately. Sending a stacked group replaces BASIC and hides the whole group.
{
  ...
  assert.equal(wormholeIsOpen(engine, "p1", ["AUG-056"]), true);

  const sent = applyWormholeTurn(engine, "p1", first.groupId, ["AUG-056"]);
  ...
  const returned = resolveDueWormholeReturns(sent, "p1", ["AUG-056"], () => 0);
  ...
  assert.equal(wormholeIsOpen(returned, "p1", ["AUG-056"]), false);

  returned.round = 3;
  assert.equal(wormholeIsOpen(returned, "p1", ["AUG-056"]), true);
}
...
// If an opening is not used in its round, it expires and the next opening is two rounds later.
{
  const engine = createInitialEngine(seeds);
  armWormholeOnAcquisition(engine, "p1");
  const expired = expireWormholeForCurrentRound(engine, "p1", ["AUG-056"]);
  assert.equal(expired.augmentRuntime?.p1?.wormholeNextOpenRound, 3);
  assert.equal(wormholeIsOpen(expired, "p1", ["AUG-056"]), false);
  expired.round = 3;
  assert.equal(wormholeIsOpen(expired, "p1", ["AUG-056"]), true);
}
...
// Finish is a legal random return candidate when the 3-18 range reaches the finish.
{
  ...
  armWormholeOnAcquisition(engine, "p1");
  const sent = applyWormholeTurn(engine, "p1", piece.groupId, ["AUG-056"]);
  sent.round = 2;
  sent.currentSeat = 1;
  sent.stage = "AWAITING_ROLL";
  sent.pendingRolls = ["BASIC"];
  const returned = resolveDueWormholeReturns(sent, "p1", ["AUG-056"], () => 0.999999);
...
// With Moonwalk, "forward" follows the owner's reversed progress direction and may return home.
{
  ...
  armWormholeOnAcquisition(engine, "p1");
  const sent = applyWormholeTurn(engine, "p1", piece.groupId, ["AUG-056", "AUG-031"]);
  sent.round = 2;
  sent.currentSeat = 1;
  sent.stage = "AWAITING_ROLL";
  sent.pendingRolls = ["BASIC"];
  const returned = resolveDueWormholeReturns(sent, "p1", ["AUG-056", "AUG-031"], () => 0);
...
console.log("[batch4-probe] PASS: AUG-056 wormhole mechanics validated.");
